...
const RbsOperasionalForm = () => {
   ...
   useEffect(() => {
    ...
    const jenisOptions = [
        ...
        { value: 'Biaya Buruh', label: 'Biaya Buruh' },
        { value: 'Meal Buruh', label: 'Meal Buruh' },
        { value: 'Meal Lembur', label: 'Meal Lembur' },
    ...
    const handleInputChange = (index, field, value) => {
        ...
        const updatedReimbursements = reimbursements.map((item, i) => {
            ...
            if (field === 'biaya' && isBbmJenis(item)) {
    ...
    const handleJenisChange = (index, selectedOption) => {
        ...
        if (selectedOption && selectedOption.value === 'Lainnya') {
            ...
            const isBbm = !!BBM_PRICE_PER_LITER[selectedOption?.value]
            const currentPlat = updatedReimbursements[index].plat
            ...
            updatedReimbursements[index] = {
                ...
                isLainnya: false,
                jenisLain: '',
                liter: isBbm ? calculateLiter(updatedReimbursements[index].biaya, selectedOption?.value) : updatedReimbursements[index].liter,
                plat: isBbm ? defaultPlat : updatedReimbursements[index].plat
            }
        }
    ...
    const generateDisplayId = async () => {
        ...
        const counterRef = doc(db, 'businessUnitCounters', `${unitCode}_RBS_OPR`)
        ...
        return runTransaction(db, async (transaction) => {
            ...
            const newDisplayId = `RBS.OPR.${unitCode}.${year.slice(-2)}${month}${day}.${newLastNumber.toString().padStart(4, '0')}`
    ...
    const uploadAttachments = async (files, displayId) => {
        ...
        try {
            ...
            return await uploadPdfFile(storage, `Reimbursement/Operasional/${displayId}/${mergedFile.name}`, mergedFile)
        } catch (error) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            // Validasi seragam untuk semua role
            ...
            // --- 3. PERCABANGAN LOGIKA SIMPAN vs UPDATE ---
            if (isEditMode) {
                ...
                let updateData = {
                    ...
                    statusHistory: arrayUnion({
                        ...
                        timestamp: new Date().toISOString(),
                        actor: userData.uid,
                        reason: 'Super Admin mengedit detail form Operasional'
                    })
                }

                // Update lampiran hanya jika Super Admin memasukkan file baru
                ...
                toast.success('Reimbursement Operasional berhasil diperbarui!')
