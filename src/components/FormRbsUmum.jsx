...
const RbsUmumForm = () => {
   ...
   useEffect(() => {
    ...
    const generateDisplayId = async () => {
        ...
        const counterRef = doc(db, 'businessUnitCounters', `${unitCode}_RBS_GAU`)
        ...
        const attemptGenerate = (offset) => runTransaction(db, async (transaction) => {
            ...
            const newDisplayId = `RBS.GAU.${unitCode}.${year.slice(-2)}${month}${day}.${candidateNumber.toString().padStart(4, '0')}`
    ...
    // supaya tombol "Lihat Lampiran" di Detail selalu membuka 1 file gabungan ---
    const uploadAttachments = async (files, displayId) => {
        ...
        try {
            ...
            return await uploadPdfFile(storage, `Reimbursement/GA_Umum/${displayId}/${mergedFile.name}`, mergedFile)
        } catch (error) {
