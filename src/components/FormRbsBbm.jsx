...
const RbsBbmForm = () => {
    ...
    const generateDisplayId = async () => {
        ...
        // displayIdOwners (mis. dokumen lama dari sebelum sistem counter ini
        // ada, ditautkan lewat backfillDisplayIdOwners() TANPA ikut memajukan
        // counter). Kalau nomor hasil counter kebetulan sudah dipakai,
        // transaction.set() ke displayIdOwners ditolak (rule-nya `allow
        // update: if false`) DAN counter tidak pernah maju karena
        // transaksinya sendiri gagal -- percobaan berikutnya menghasilkan
        // nomor yang SAMA lagi, terjebak loop gagal permanen untuk unit itu.
        // TIDAK membaca displayIdOwners sama sekali (rule-nya melarang total
        // get/list) -- coba tulis, kalau ditolak karena nomornya sudah
        // dipakai, retry DI LUAR transaksi dengan offset nomor berikutnya.
        const attemptGenerate = (offset) => runTransaction(db, async (transaction) => {
            ...
            const newDisplayId = `RBS.BBM.${unitCode}.${year.slice(-2)}${month}${day}.${candidateNumber.toString().padStart(4, '0')}`
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditMode) {
                ...
                // (dari editData.user), bukan uid editor yang login -- pakai
                // localStorage langsung supaya statusHistory mencatat siapa
                // sebenarnya yang melakukan perubahan.
