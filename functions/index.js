...
// Bagian BD: diagnostik SEMENTARA, khusus Super Admin -- membaca nilai mentah
// nomorBS/displayId (di-escape lewat JSON.stringify supaya spasi/karakter
// tersembunyi terlihat) untuk 1 kasus BS yang statusnya masih "Belum LPJ"
// walau normalisasi (Bagian BB/BC) sudah diterapkan, artinya perbedaannya
// kemungkinan besar bukan cuma spasi/kapitalisasi. Read-only, TIDAK mengubah
// data apa pun -- dan dihapus lagi segera setelah dipakai untuk diagnosis.
exports.debugFindLpjMismatch = onCall(async (request) => {
    if (!request.auth?.uid) {
    ...
    const targetDisplayId = normalizeText(request.data?.displayId, "displayId", 200);

    const bsSnapshot = await db.collection("bonSementara").where("displayId", "==", targetDisplayId).get();
    const bsResults = bsSnapshot.docs.map((docSnap) => ({
        id: docSnap.id,
        displayId: JSON.stringify(docSnap.data().displayId),
        status: docSnap.data().status,
        uid: docSnap.data().user?.uid
    }));

    const ownerUids = new Set(bsSnapshot.docs.map((docSnap) => docSnap.data().user?.uid).filter(Boolean));

    const lpjSnapshot = await db.collection("lpj").get();
    const lpjAll = lpjSnapshot.docs.map((docSnap) => ({
        id: docSnap.id,
        nomorBS: docSnap.data().nomorBS,
        status: docSnap.data().status,
        uid: docSnap.data().user?.uid
    }));

    const b = targetDisplayId.toString().trim().toUpperCase();
    const lpjSimilar = lpjAll
        .filter((item) => {
            if (!item.nomorBS) return false;
            const a = item.nomorBS.toString().trim().toUpperCase();
            return a === b || a.includes(b) || b.includes(a);
        })
        .map((item) => ({ ...item, nomorBS: JSON.stringify(item.nomorBS) }));

    const lpjSameOwner = lpjAll
        .filter((item) => ownerUids.has(item.uid))
        .map((item) => ({ ...item, nomorBS: JSON.stringify(item.nomorBS) }));

    return { bsResults, lpjSimilar, lpjSameOwner };
});
